...
static void dump_instr_ab(struct fh_output *out, uint32_t instr)
{
  int a = GET_INSTR_RA(instr);
  int b = GET_INSTR_RB(instr);
...
static void dump_instr(struct fh_bc *bc, struct fh_output *out, int32_t addr, uint32_t instr)
{
  ...
  case OPC_SUB:   fh_output(out, "sub     "); dump_instr_abc(out, instr); return;
  case OPC_MUL:   fh_output(out, "mul     "); dump_instr_abc(out, instr); return;
  case OPC_DIV:   fh_output(out, "div     "); dump_instr_abc(out, instr); return;
  case OPC_MOD:   fh_output(out, "mod     "); dump_instr_abc(out, instr); return;
  case OPC_MOV:   fh_output(out, "mov     "); dump_instr_ab(out, instr); return;
